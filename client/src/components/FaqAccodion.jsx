  ...
  return (
    ...
    <>
        <div className="my-5 bg-[#FFFFFF] shadow-lg accodion rounded-md rounded-t-md "> 
            <button onClick={toggle} className={`flex  focus:outline-none focus:ring focus:ring-[#BAD3F6] w-full justify-between items-center px-3 rounded-md ${open? 'shadow-orange-300 rounded-md focus:ring-0 transition' :''}`}>
                <span className={`py-5 font-[poppins] font-bold text-start ${open ? ' text-[#ff6600]' : ''}`}>{title}</span> 
                <div className={`${open ? ' text-[#ff6600]' : ''}`}>
                    {
                        open ?  <Minus /> : <Plus /> 
                    }
                </div>
